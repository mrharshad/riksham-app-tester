...
const UpdateProduct = ({ token }) => {
  ...
  const findProducts = async (event) => {
    ...
    const productData = await fetch("/api/product-search/keyword-products", {
      method: "POST",
  ...
  const removeProduct = (_id) => {
    setProducts((previous) =>
      previous.filter((product) => product._id !== _id)
    );
    setUpdateProduct();
    setContainer();
  };
  const openContainer = (data, name) => {
    setContainer(name);
    setUpdateProduct(data);
  };
  return (
    <Fragment>
      <div id="productManagersPanels" className={style.mainContainer}>
        {wait ? <Wait /> : null}
        <form className={style.searchBar} onSubmit={findProducts}>
        ...
        <div className={style.showDetails}>
          ...
          <p className={style.updateIcon}> D</p>
          <p className={style.deleteKey}> V / O</p>
        </div>
          ...
          {products.map((data, index) => {
            return (
              <div key={index} className={style.products}>
                ...
                {container ? null : (
                  <svg
                    onClick={() => openContainer(data, "updateD")}
                ...
                {container ? null : (
                  <svg
                    onClick={() => openContainer(data, "updateV")}
                    className={style.updateIcon}
                    x="0px"
                    y="0px"
                    width="40"
